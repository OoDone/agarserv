...
var playerCommands = {
    ...
    playerlist: function (gameServer, split) {
        if (!gameServer.clients.length) return this.writeLine("No bots or players are currently connected to the server!");
        this.writeLine("\nCurrent players: " + gameServer.clients.length);
        this.writeLine('Do "playerlist m" or "pl m" to list minions\n');
        this.writeLine(" ID     | IP              | P | CELLS | SCORE  |   POSITION   | " + fillChar('NICK', ' ', gameServer.config.playerMaxNickLength) + " "); // Fill space
        this.writeLine(fillChar('', '─', ' ID     | IP              | CELLS | SCORE  |   POSITION   |   |  '.length + gameServer.config.playerMaxNickLength));
        ...
        for (var i = 0; i < sockets.length; i++) {
            ...
            if (socket.closeReason != null) {
                ...
                this.writeLine(" " + id + " | " + ip + " | " + protocol + " | " + "[CONNECTING]");
